...
use rand::{rand_core, rngs::OsRng, Rng, TryRngCore};
...
impl Scalar {
    ...
    */
    ...
    /// Generates a random non-null scalar.
    /// This should be secure for secret key and nonce generation.
    /// This function is not constant time: it rejects and retries if the
    /// result is greater than the field order.
    pub fn random() -> Result<Scalar, rand_core::OsError> {
        let mut rng = OsRng;
        let mut k = [0u8; 40];
        loop {
            rng.try_fill_bytes(&mut k)?;
            // if k overflows, decode will return 0, that we want to reject
            // in any case
            let (k_scalar, _) = Scalar::decode(&k);
            // if k_scalar != 0 return, else retry
            if k_scalar.iszero() == 0 {
                return Ok(k_scalar);
            }
        }
    }

    /// Generates a random non-null scalar based on the provided Rng
    /// This function is not constant time: it rejects and retries if the
    /// result is greater than the field order.
    ...
    /// converts a GFp5 into a scalar, by reducing modulo in case of overflow
    pub fn from_gfp5(x: &GFp5) -> Self {
        Self::decode_reduce(&x.encode())
    }
